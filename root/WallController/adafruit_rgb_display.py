...
class DisplaySPI(Display):
    ...
    def __init__(
        ...
        *,
        ...
        self.dc_pin = chip.get_line(dc)
        self.rst = chip.get_line(rst)
        self.dc_pin.request(consumer='root', type=gpiod.LINE_REQ_DIR_OUT)
        if self.rst:
            self.rst.request(consumer='root', type=gpiod.LINE_REQ_DIR_OUT)
            self.reset()
    ...
    def write(self, command=None, data=None):
        ...
        if data is not None:
            ...
            if isinstance(data, (bytes, bytearray)) and len(data)>0:
                ...
                bytes_content = bytearray(data)
                for byte in bytes_content:
                    print(byte)
                    send_data = send_data + (byte,)
                print(send_data)
            else:
                send_data = data
            self.spi_device.writebytes(send_data)
    ...
    def read(self, command=None, count=0):
        ...
        #with self.spi_device as spi:
        if command is not None:
            self.spi_device.writebytes(bytearray([command]))
        if count:
